...
import static com.bearingpoint.infonova.jenkins.listener.ResourceBpmnParseListener.JENKINS_TASK_PROPERTY;
...
public final class TaskHighlightFactory {
	 ...
	 */
	public static AbstractTaskHighlight getObject(ActivityImpl activity, int xEff, int yEff,
			AbstractBuild<?, ?> build) {
		...
		if (isJenkinsTask(activity)) {
			String jobName = (String) activity.getProperty(JENKINS_TASK_PROPERTY);
			return new JenkinsActivitiTaskHighlight(processId, activity.getId(), x1, y1, x2, y2,
					jobName);
		}
	...
	private static boolean isJenkinsTask(ActivityImpl activity) {
		return activity.getProperty(JENKINS_TASK_PROPERTY) != null;
	}
