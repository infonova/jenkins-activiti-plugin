...
public final class CallActivityHighlightFactory {
    ...
    public static CallActivityTaskHighlight getObject(ActivityImpl activity, int xEff, int yEff,
        ...
        if (ActivitiUtils.isCallActivity(activity)) {
            final String key = (String)activity.getProperty(CustomCallActivityParseHandler.CALLACTIVITY_PROPERTY);
            ...
            CallActivityTaskHighlight callActivity = new CallActivityTaskHighlight(processId, activity.getId(), x1, y1,
                x2, y2, definition, picture);
