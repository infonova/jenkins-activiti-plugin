...
@PrepareOnlyThisForTest({
    ActivitiAccessor.class, TaskHighlightFactory.class, EventHighlightFactory.class, GatewayHighlightFactory.class,
...
public class ActivitiUtilsUTest {
    ...
    public void testGetDiagramResourceAsStreamExpectErrorCodeActiviti01() {
        ...
        PowerMockito.when(ActivitiAccessor.getProcessDefinitionById(Matchers.anyString())).thenReturn(definition);
    ...
    public void testGetAreas() {
        ...
        TaskHighlightFactory.getObject(Matchers.any(ActivityImpl.class), Matchers.anyInt(), Matchers.anyInt(), Matchers
            .eq(build));

        EventHighlightFactory.getObject(Matchers.any(ActivityImpl.class), Matchers.anyInt(), Matchers.anyInt());

        GatewayHighlightFactory.getObject(Matchers.any(ActivityImpl.class), Matchers.anyInt(), Matchers.anyInt());

        CallActivityHighlightFactory.getObject(Matchers.any(ActivityImpl.class), Matchers.anyInt(), Matchers.anyInt(),
            Matchers.eq(build));
    }
    ...
    public void testAddActivityListeners() {
        ...
        PowerMockito.when(ActivitiAccessor.getProcessDefinitionEntity("subProcessId")).thenReturn(entity);
        ...
        for (ActivityImpl activity : activities) {
            Mockito.verify(activity).addExecutionListener(Matchers.anyString(), Matchers.any(ExecutionListener.class));
            Mockito.verify(activity).addExecutionListener(Matchers.anyString(), Matchers.any(ExecutionListener.class),
                Matchers.eq(0));
        }
    }
    ...
    private final ActivityImpl mockActivity() {
        ActivityImpl activity = Mockito.mock(ActivityImpl.class);
        Mockito.when(activity.getProperty(CustomScriptTaskParseHandler.SCRIPT_PROPERTY)).thenReturn("script");
    ...
    private final ProcessDefinition mockProcessDefinition() {
        ProcessDefinition definition = Mockito.mock(ProcessDefinition.class);
        PowerMockito.when(ActivitiAccessor.getProcessDefinitionById("processDefinition")).thenReturn(definition);
    ...
    private final List<ActivityImpl> mockActivities(String processDefinition) {
        ProcessDefinitionEntity entity = Mockito.mock(ProcessDefinitionEntity.class);
        PowerMockito.when(ActivitiAccessor.getProcessDefinitionEntity(processDefinition)).thenReturn(entity);
    ...
    private final List<ActivityImpl> mockActivities() {
        ProcessDefinitionEntity entity = Mockito.mock(ProcessDefinitionEntity.class);
        PowerMockito.when(ActivitiAccessor.getProcessDefinitionEntity(Matchers.anyString())).thenReturn(entity);
